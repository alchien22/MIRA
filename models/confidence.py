...
def compute_confidence_score(response_latents, retrieved_latents, base_confidence, use_rag):
    """Compute a composite confidence score combining base confidence and retrieval similarity."""
    if not use_rag:
...
def compute_entropy_confidence(logits, max_entropy=5):
    """Computes base confidence based on entropy (lower entropy = higher confidence)."""
...
def compute_perplexity_confidence(logits):
    """Computes base confidence using perplexity (lower perplexity = higher confidence)."""
    probs = torch.nn.functional.softmax(logits, dim=-1)
...
def compute_dynamic_lambda(retrieved_latents, response_latents):
    """Dynamically adjust λ based on retrieval quality."""
...
def batch_extract_latents(model, tokenizer, texts, confidence_method="entropy"):
    ...
    return [embedding.tolist() for embedding in pooled_embeddings]  # Convert to list
